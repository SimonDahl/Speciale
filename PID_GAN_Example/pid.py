...
class Burgers_PID():
    def __init__(self, x_u, y_u, x_f, X_star, u_star, G, D, Q, device, nepochs, lambda_val, noise = 0.0):
...
#         U_pred = griddata(X_star, u_pred.flatten(), (X, T), method='cubic')
    ...
    def train_generator(self, x, y):
        ...
        for gen_epoch in range(5):
            ...
            # physics loss for collocation points
            n_phy, phyloss_1, u_f, _ = self.n_phy_prob(self.train_x_f)
            fake_logits_f = self.D.forward(torch.cat([self.train_x_f, u_f, n_phy], dim=1))
            # physics loss for boundary points
            n_phy, phyloss_2, y_pred, G_noise = self.n_phy_prob(x)
