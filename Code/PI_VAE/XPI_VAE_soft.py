...
sol_data = np.zeros((n_data,timesteps*2))
...
for i in range(n_data):
    ...
    sol_data[i,:timesteps] = solution.y[0]
    sol_data[i,timesteps:] = t
...
class VAE(nn.Module):
    ...
    def forward(self, x):
        mu, log_var = self.encoder(x.view(-1, timesteps*2))
...
vae = VAE(x_dim=timesteps*2, h_dim1=h1, h_dim2=h2,h_dim3=h3,h_dim4=h4, z_dim=z_dim_size)


def compute_residuals(recon_x):
    
    print(recon_x.shape)
    
    u = recon_x[:,0:timesteps]
    x = recon_x[:,timesteps:]
    
    
    u = u.reshape(timesteps,1)
    x = x.reshape(timesteps,1)
...
def loss_function(recon_x, x, mu, log_var,x_col):
    MSE = F.mse_loss(recon_x, x.view(-1, timesteps*2), reduction='sum')
