...
def D_train(x,y_train):
    ...
    real_logits = torch.mean(real_logits)
    ...
    fake_logits_u = torch.mean(fake_logits_u)
...
def Q_train(x):
    ...
    y_pred = torch.mean(y_pred)
    y_pred = y_pred.reshape(1)
    zp_input = torch.concat((x,y_pred))
