...
lambda_phy = 10 # physics loss weight
...
def sho(t,y):
    solution = (y[1],(-(c/m_known)*y[1]-(k_known/m_known)*y[0])) # damped harmonic oscillator 
    return solution
...
class Discriminator(nn.Module):
    ...
    def forward(self, d):
        ...
        return torch.sigmoid(self.fc3(d)) 
...
def G_train(t_train,u_real,x_collocation):
    ...
    L_adv = torch.mean(-torch.log(D(d_input)+1e-8)) 
    ...
    m_approx,k_approx = G.getODEParam()
    k_approx = k_approx.detach().numpy()
    m_approx = m_approx.detach().numpy()
    
    return G_loss,m_approx,k_approx
...
def D_train(t_train,u_real):
    ...
    for d_epoch in range(D_epochs):
        ...
        u_fake = G(t_train)
        d_input = torch.cat((t_train,u_fake),dim=1)
        u_fake_prob = D(d_input)

        d_input = torch.cat((t_train,u_real),dim=1)
        u_real_prob = D(d_input)
        
        D_loss = torch.mean(-torch.log(u_real_prob+1e-8)+torch.log(1-u_fake_prob+1e-8))
...
for epoch in range(1, n_epochs+1):
    D_losses, G_losses,m_batch,k_batch = [], [],[],[]
    batch = 0
    for batch_idx in range(train_size//bs):
        ...
        G_loss,m,k = G_train(t_batch,u_batch,x_col)
    ...
    m_approx[epoch-1]=(np.mean(m_batch))
    k_approx[epoch-1]=(np.mean(k_batch))
