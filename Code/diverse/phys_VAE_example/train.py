...
def loss_function(args, data, omega_stat, z_aux1_stat, z_aux2_stat, x_mean):
    n = data.shape[0]
    device = data.device

    recerr_sq = torch.sum((x_mean - data).pow(2), dim=1).mean()

    prior_omega_stat, prior_z_aux1_stat, prior_z_aux2_stat = model.priors(n, device)
    ...
    KL_omega = utils.kldiv_normal_normal(omega_stat['mean'], omega_stat['lnvar'],
        prior_omega_stat['mean'], prior_omega_stat['lnvar']) if not args.no_phy else torch.zeros(1, device=device)
...
def train(epoch, args, device, loader, model, optimizer):
    ...
    for batch_idx, (data,) in enumerate(loader):
        ...
        omega_stat, z_aux1_stat, z_aux2_stat, unmixed = model.encode(data)
        ...
        # ELBO
        recerr_sq, kldiv = loss_function(args, data, omega_stat, z_aux1_stat, z_aux2_stat, x_PAB)
...
def valid(epoch, args, device, loader, model):
    ...
    with torch.no_grad():
        for i, (data,) in enumerate(loader):
            data = data.to(device)
            batch_size = len(data)
            omega_stat, z_aux1_stat, z_aux2_stat, x_mean, _ = model(data)
            recerr_sq, kldiv = loss_function(args, data, omega_stat, z_aux1_stat, z_aux2_stat, x_mean)
