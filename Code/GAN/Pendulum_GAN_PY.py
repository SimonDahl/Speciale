...
def compute_residuals(u):
    ...
    u_t  = torch.autograd.grad(u, t_diff, torch.ones([timesteps,1]), allow_unused=True ,retain_graph=True,create_graph=True)[0]# computes dy/dx
    
    u_tt = torch.autograd.grad(u_t,  t_diff, torch.ones([timesteps,1]),allow_unused=True,retain_graph=True ,create_graph=True)[0]# computes d^2y/dx^2
...
def G_train(x):
    ...
    res = compute_residuals(G_output)
    phy_loss = (res**2).mean()
    
    G_loss = criterion(D_output, y) + lambda_phy* phy_loss
