...
plt.figure()
plt.grid()
plt.title("Lotka_Volterra")
plt.scatter(t, x[0,0:timesteps],label = 'Deer')
plt.scatter(t, x[0,timesteps:],label = "Wolves")
...
with torch.no_grad():
    ...
    for i in range(0,2):
        for j in range(0,4):
            ...
            ax[i,j].plot(t,y[0:t_gen])
            ax[i,j].plot(t,y[t_gen:])
    ...
    #plt.show()
    plt.savefig('./output/GAN/LV/'+'n_epochs ' +str(n_epochs)+' z_dim_size '+str(z_dim)+' lr '+str(lr)+'.png')     
